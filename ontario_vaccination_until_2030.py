...
year_to_repeat_until_2030 = 2020

# path to outputs
path_to_output_csv_new_infection = 'C:/post_doc/data/output_sim/emily/ontario_project/new_infection_per_cell_vaccine_until_2030.csv'
path_to_output_csv_death_from_rabies = 'C:/post_doc/data/output_sim/emily/ontario_project/death_by_rabies_vaccine_until_2030.csv'
...
with open(path_to_vaccination_csv, 'r') as f_vac:
    for i, line in enumerate(f_vac):
        ...
        if int(data[index_year]) == year_to_repeat_until_2030:
            for j in range(1, 11):
                timestep = 52 * (int(data[index_year]) + j) + (int(data[index_week]) - 1) - initial_timestep
                if timestep in dict_timestep_to_vac:
                    try:
                        current_level = dict_timestep_to_vac[timestep][data[index_hex_id]]
                        if float(data[index_level]) > current_level:
...
for i in range(nb_year_simu * 52 + 1):
    ...
    if racoons.df_population['imm_rabies'].sum() == 0:
        break
